        ...
        >
            ...
            <div className="flex flex-1 min-h-0 w-full overflow-hidden">
                        ...
                        <div className="bg-slate-900 border-t border-slate-700/85 px-6 py-4 flex flex-col gap-3 shadow-2xl relative select-none">
                            ...
                            <div className="flex items-center justify-between border-b border-slate-800/80 pb-2">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex items-center bg-slate-950/40 rounded-lg p-0.5 border border-slate-800">
                                        <button
                                            ...
                                            className={`px-3 py-1.5 rounded text-[10px] font-bold transition-all flex items-center gap-1.5 disabled:opacity-40 disabled:pointer-events-none ${animPlaying
                                ...
                                <div className="flex items-center gap-3">
                                    {/* Auto Key Switch */}
                                    <label className={`flex items-center gap-2 px-3 py-1 rounded-lg border transition-all cursor-pointer select-none ${autoRecord
                                    ...
                                    <div className="flex-1 flex items-center gap-1.5 overflow-x-auto py-1.5 custom-scrollbar">
                                        {keyframes.map((_, idx) => (
                                            <button
                                                ...
                                                className={`group relative min-w-8 h-8 rounded border flex flex-col items-center justify-center transition-all ${selectedKeyframe === idx
                                            ...
                                            >
                                                ...
                                                <span className={`w-1.5 h-1.5 rotate-45 border mt-0.5 ${selectedKeyframe === idx
                ...
                <div className="w-[300px] bg-slate-800 p-5 flex flex-col gap-4 shadow-xl z-20 border-l border-slate-700 overflow-y-auto custom-scrollbar">
                    ...
                    <label className="block text-[11px] font-bold text-amber-400 mb-3 border-b border-slate-600 pb-2">{uiMode === 'PLAYER_PREVIEW' ? 'Player Controls' : 'Selection Properties'}</label>
                    {uiMode === 'PLAYER_PREVIEW' ? (
                        <div className="space-y-4 flex-1 overflow-y-auto custom-scrollbar pr-2">
                            <span className="text-[10px] text-emerald-400 font-bold block mb-1">RUNTIME PREVIEW</span>
                            <div className="grid grid-cols-2 gap-2">
                                <button onClick={() => playerPreviewRef.current?.play?.()} className="py-1.5 rounded bg-emerald-600 hover:bg-emerald-500 text-[10px] font-bold text-white shadow">Play</button>
                                <button onClick={() => playerPreviewRef.current?.stop?.()} className="py-1.5 rounded bg-amber-600 hover:bg-amber-500 text-[10px] font-bold text-white shadow">Stop</button>
                            </div>
                            <div>
                                <label className="text-[9px] text-slate-400 block mb-1">Animation</label>
                                <select className="bg-slate-800 text-[10px] rounded border border-slate-600 px-2 py-1.5 w-full text-slate-300" onChange={(e) => playerPreviewRef.current?.setAnimation?.(e.target.value)} defaultValue={currentAnimationTitle}>
                                    {[...animations.filter(anim => anim.title !== currentAnimationTitle), buildCurrentAnimation()].map(anim => (
                                        <option key={anim.title} value={anim.title}>{anim.title}</option>
                                    ))}
                                </select>
                            ...
                            <div>
                                <label className="text-[9px] text-slate-400 block mb-1">Scale Mode</label>
                                <select className="bg-slate-800 text-[10px] rounded border border-slate-600 px-2 py-1.5 w-full text-slate-300" onChange={(e) => playerPreviewRef.current?.setScaleMode?.(e.target.value)} defaultValue="contain">
                            ...
                            <div>
                                <label className="text-[9px] text-slate-400 block mb-1">Anchor</label>
                                <select className="bg-slate-800 text-[10px] rounded border border-slate-600 px-2 py-1.5 w-full text-slate-300" onChange={(e) => playerPreviewRef.current?.setAnchor?.(e.target.value)} defaultValue="center">
                            ...
                            <div>
                                <div className="flex justify-between text-[9px] text-slate-400 mb-1"><span>Mesh Density</span></div>
                                <input type="range" min="0.1" max="2" step="0.1" defaultValue="1" onChange={(e) => playerPreviewRef.current?.setMeshDensity?.(Number(e.target.value))} className="w-full accent-emerald-500" />
                            </div>
                            <div>
                                <div className="flex justify-between text-[9px] text-slate-400 mb-1"><span>Animation Speed</span></div>
                                <input type="range" min="0" max="4" step="0.1" defaultValue="1" onChange={(e) => playerPreviewRef.current?.setAnimationSpeed?.(Number(e.target.value))} className="w-full accent-emerald-500" />
                            </div>
                            <div>
                                <div className="flex justify-between text-[9px] text-slate-400 mb-1"><span>Display Scale</span></div>
                                <input type="range" min="0.1" max="3" step="0.1" defaultValue="1" onChange={(e) => playerPreviewRef.current?.setDisplayScale?.(Number(e.target.value))} className="w-full accent-emerald-500" />
                            </div>
                            <div>
                                <label className="flex items-center gap-2 text-[10px] cursor-pointer text-slate-200 font-bold bg-slate-800 px-2 py-1.5 rounded shadow">
                                    <input type="checkbox" defaultChecked={true} onChange={(e) => playerPreviewRef.current?.setLissajousEnabled?.(e.target.checked)} className="rounded bg-slate-800 border-emerald-500" /> Enable Lissajous
                                </label>
                            </div>
                            <div>
                                <label className="text-[9px] text-slate-400 block mb-1">Debug Mode</label>
                                <select className="bg-slate-800 text-[10px] rounded border border-slate-600 px-2 py-1.5 w-full text-slate-300" onChange={(e) => playerPreviewRef.current?.setDebug?.(e.target.value === 'false' ? false : e.target.value === 'true' ? true : Number(e.target.value))} defaultValue="false">
                                    <option value="false">Off</option>
                                    <option value="true">On (Full)</option>
                                    <option value="0">0: Bones</option>
                                    <option value="1">1: Jiggles/Pins</option>
                            ...
                            <div className="space-y-4 flex-1">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <button onClick={() => playerPreviewRef.current?.play?.()} className="py-1.5 rounded bg-emerald-600 hover:bg-emerald-500 text-[10px] font-bold text-white shadow">Play</button>
                                    <button onClick={() => playerPreviewRef.current?.pause?.()} className="py-1.5 rounded bg-amber-600 hover:bg-amber-500 text-[10px] font-bold text-white shadow">Pause</button>
                                </div>
                                <div>
                                    <label className="text-[9px] text-slate-400 block mb-1">Animation</label>
                                    <select className="bg-slate-800 text-[10px] rounded border border-slate-600 px-2 py-1.5 w-full text-slate-300" onChange={(e) => playerPreviewRef.current?.setAnimation?.(e.target.value)} defaultValue={currentAnimationTitle}>
